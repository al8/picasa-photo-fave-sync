...
def run(params, path, dirs, files, logger=None):
    ...
    for s in config.sections():
        if logger:
            logger.debug("%s %s %s" % (s, config.items(
                s), ('suppress', 'yes') in config.items(s)))
        if ('suppress', 'yes') in config.items(s):  # "Block from Uploading" flag in picasa
            cnt_suppressed += 1
            continue
        if logger:
            logger.debug("%s %s %s" % (s, config.items(
                s), ('star', 'yes') in config.items(s)))
    ...
    if logger and (len(files) > 0 or cnt_suppressed > 0):
        ...
        logger.debug("picasa_ini filter: %d files starred%s, %d left in \"%s\"" % (
            len(star_files), str_suppressed, len(files), os.path.basename(path)))
    return dirs, files
