...
def get_files(transfer_param, path_override=None):
    ...
    # if len(p_dirs) == 0 and len(p_files) == 0 else g_lgr.info
    logger = g_lgr.debug
...
def copy_resize_rotate(files, output_path):
    ...
    for idx, f in enumerate(sorted(files)):
        ...
        if os.path.isfile(dst):
            g_lgr.debug(
                "skipping file '%s' because dst:'%s' already exists" % (src, dst))
            cnt_skip += 1
        ...
        if not src.lower().endswith("jpg"):
            g_lgr.error(
                "skipping file '%s' because it is not a jpeg!!!" % (src))
            continue
        ...
        g_lgr.debug("resizing file '%s' to '%s' (%d of %d)" %
                    (src, dst, idx + 1, len(files)))
        ...
        if g_params["jhead_binary"]:
            ...
            try:
                ...
                g_lgr.warning(
                    "removing file that could not be rotated %s" % dst)
    ...
    if cnt_convert:
        g_lgr.info("Resized %d files (%d skipped)" % (cnt_convert, cnt_skip))
...
def cleanup_output_path(output_path, output_files):
    ...
    if len(cleanup_files):
        g_lgr.info("number of files to clean up: %d" % len(cleanup_files))
...
def remote_delete_files(host, port, files):
    ...
    g_lgr.info("num remote files deleted '%s' (requested:%d)" %
               (recv, len(files)))
    return recv
